...
function SuggestionDish({suggestionDish,item,handleItemBackground,AddDish,clickedItems,index,setClickedItems,searchIndex,setSearchIndex}) {
  ...
  return (
    ...
    <div className="bg-primaryColor flex items-center justify-between p-3 rounded-t-md">
      ...
      >
        {
          searchIndex == index  ? ( <div className="">
          <input
            type="text"
      ...
      <div className="border">
  {suggestionDish[item]?.filter((item1) => (item1.name?.toLowerCase()?.includes(searchQuery.toLowerCase())))?.length === 0 ? (
    <p>Dish not found</p>
  ) : (
    suggestionDish[item]?.filter((item1) => (item1.name?.toLowerCase()?.includes(searchQuery.toLowerCase())))?.map((dish, index) => {
      const itemBackgroundColor =
